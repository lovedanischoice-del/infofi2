...
const ImportantDates: React.FC<ImportantDatesProps> = ({ importantDates, onDelete, onOpenModal, canEdit }) => {
    ...
    return (
        <div className="relative">
            {importantDates.length > 0 && canEdit && (
                <button
                    onClick={onOpenModal}
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                {sortedDates.map((d) => (
                    <div key={d.id} className={`flex items-center justify-between bg-white/50 p-2 rounded-md ${canEdit ? 'group' : ''}`}>
                ...
                {importantDates.length === 0 && !canEdit && (
                    <p className="text-center text-sm text-medium py-2">No important dates added.</p>
                )}
